...
This removes the need for a module to add core-table columns for
...
module-owned models and tables. Examples include catalogues, histories,
decisions, memory, reports, and analytics.
...
overlap lock. Scheduled maintenance and batch orchestration belong here.
...
This is for game-timed state tied to update ordering, such as local construction
or timed resource updates. It is not a replacement for a Laravel background worker.
...
A complex module combines the extension points rather than relying on a single
one: it can register game objects, use normalized object state, own relational
tables for its domain data, extend production, run a planet-bound processor,
add settings and pages, and react to after-commit events. Its primary scaling
concerns are bulk loading of module object state and deterministic processing
inside the planet update transaction.

A module that performs background work can own profiles, strategies, cooldowns,
decision logs, and memory in module tables. Scheduled commands and regular
Laravel jobs perform planning; normal game services execute legal actions;
after-commit events trigger follow-up work.

The remaining core prerequisites for such modules are a public player-state
advancement service for non-HTTP ticks and stable public APIs for discovering
legal available actions. A module should not bypass domain services by directly
...
4. Stable available-action APIs for automation modules.
