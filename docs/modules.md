...
```php
$seconds = $settings->module('mymodule')->integer('tick_seconds');
```
...
would be unsafe. Scheduled maintenance and batch work are good scheduled-command use cases.

### Planet-bound queues

Use `ModuleQueues` and a registered `ProvidesQueueProcessor` for deterministic
work that must run in the normal planet update transaction, such as local
construction. Keep processors synchronous, fast, idempotent, and free of remote I/O.
...
For technical design, lifecycle ordering, and performance limits, see
[Module foundation and planning](planning/modules.md).
